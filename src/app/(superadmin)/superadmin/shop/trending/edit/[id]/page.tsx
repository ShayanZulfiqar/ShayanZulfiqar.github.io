...
import { GET_SHOP_TRENDING_BY_ID } from "@/services/ApiRoutes";
import { useAppDispatch } from "@/store/hooks";
import { updateTrendingProductAction } from "@/store/slices/shopSlice";

const EditShopTrending = () => {
    ...
    useEffect(() => {
        const fetchItem = async () => {
            try {
                const res = await get(GET_SHOP_TRENDING_BY_ID(id));
                if (res) {
                    // Handle potential wrapper or direct data
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await dispatch(updateTrendingProductAction({ id, data: formData })).unwrap();
            Swal.fire("Success", "Trending item updated successfully!", "success");
            router.push("/superadmin/shop/trending");
        } catch (error: any) {
            Swal.fire("Error", error?.message || "Failed to update trending item", "error");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="p-6 max-w-4xl mx-auto">
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h1 className="text-3xl font-bold text-gray-800">Edit Trending Item</h1>
                    <p className="text-gray-500 mt-2">Modify the details of this trending entry</p>
                </div>
...
export default EditShopTrending;
