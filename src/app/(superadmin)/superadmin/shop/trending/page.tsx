...
import { fetchTrendingProducts, deleteTrendingProductAction } from "@/store/slices/shopSlice";
...
const ShopTrendingList = () => {
    const dispatch = useAppDispatch();
    const { data: items, loading } = useAppSelector((state) => state.shop.trending);
    ...
    const handleDelete = async (id: string) => {
        ...
        if (result.isConfirmed) {
            try {
                const resultAction = await dispatch(deleteTrendingProductAction(id));
                if (deleteTrendingProductAction.fulfilled.match(resultAction)) {
        ...
        <div className="p-6 max-w-[1600px] mx-auto">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    ...
                    <p className="text-gray-500 mt-2">Manage products in the trending section</p>
                </div>
                <Link href="/superadmin/shop/trending/add">
                    ...
                    <table className="w-full text-left">
                        <thead className="bg-gray-50 border-b border-gray-100">
                            <tr>
                                ...
                                <th className="px-6 py-4 font-semibold text-gray-700">Products</th>
                                <th className="px-6 py-4 font-semibold text-gray-700">Rating</th>
                                <th className="px-6 py-4 font-semibold text-gray-700">Reviews</th>
                            ...
                            {loading ? (
                                ...
                                filteredItems.map((item) => (
                                    <tr key={item._id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-gray-600">{item.productsNumber}</td>
                                        <td className="px-6 py-4 text-gray-600">{item.ratingNumber}</td>
                                        <td className="px-6 py-4 text-gray-600">{item.reviewNumber}</td>
                                        <td className="px-6 py-4">
                                            <div className="flex items-center justify-end gap-2">
                                                <Link href={`/superadmin/shop/trending/edit/${item._id}`}>
                                ...
                                <tr>
                                    <td colSpan={6} className="px-6 py-12 text-center">
                                        <div className="flex flex-col items-center justify-center text-gray-400">
                                            ...
                                            <p className="text-lg font-medium">No trending items found</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
...
export default ShopTrendingList;
