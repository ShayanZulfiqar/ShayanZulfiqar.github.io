...
import { ArrowLeft, Save, Upload, Eye, X, Image as ImageIcon } from "lucide-react";
import { createClientBrandAction } from "@/store/slices/landingSlice";
import { useAppSelector, useAppDispatch } from "@/store/hooks";
import AlertDialog from "@/utils/AlertDialog";

const AddClientBrandPage = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!logoFile) {
            AlertDialog("Error!", "Please upload a logo image", "error", 3000, false, false);
            return;
        }
        ...
        try {
            ...
            const resultAction = await dispatch(createClientBrandAction(formDataToSend));

            const endTime = performance.now();
            console.log(`Create API Response Time: ${(endTime - startTime).toFixed(2)}ms`);

            if (createClientBrandAction.fulfilled.match(resultAction)) {
                AlertDialog(
                    "Success!",
                    "Client brand created successfully!",
                    "success",
                    1500,
                    false,
                    false
                ...
                throw new Error(resultAction.payload as string || "Failed to create client brand");
            }
        } catch (error: any) {
            AlertDialog(
                "Error!",
                error.message || "Failed to create client brand",
                "error",
                3000,
                false,
                false
            ...
            >
                ...
                <div>
                    ...
                    <p className="text-gray-600 flex items-center gap-2 mt-1">
                        ...
                        <span>Create a new client brand entry</span>
                    </p>
                </div>
            </motion.div>
            ...
            >
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div>
                        ...
                        />
                        <div className={`mt-2 h-16 rounded-lg border-2 border-gray-300 bg-gradient-to-r ${formData.gradient}`} style={{
                            backgroundImage: formData.gradient.includes('from-')
                            ...
                            <div className="relative border-2 border-gray-300 rounded-lg p-6 bg-gray-50">
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex-1">
                                        <p className="font-semibold text-gray-800 mb-1">{logoFile?.name}</p>
                    ...
                    <div className="flex items-center gap-4 pt-4">
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="flex-1 flex items-center justify-center gap-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white px-6 py-3 rounded-xl font-semibold transition-all shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            {loading ? (
                                <>
...
export default AddClientBrandPage;
