...
import { fetchQuestions, deleteQuestionAction } from "@/store/slices/landingSlice";
import AlertDialog from "@/utils/AlertDialog";

const GotQuestionsPage = () => {
    const router = useRouter();
    const dispatch = useAppDispatch();
    const { data: questions, loading, error } = useAppSelector((state) => state.landing.questions);
    ...
    const handleDelete = (id: string, question: string) => {
        AlertDialog(
            ...
            async () => {
                try {
                    const resultAction = await dispatch(deleteQuestionAction(id));
        ...
        return (
            <div className="min-h-screen flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-gray-600 text-lg">Loading Q&A...</p>
                </div>
            </div>
        );
    }
            ...
            >
                <div>
                    ...
                    <p className="text-gray-600 flex items-center gap-2">
                        ...
                        <span>Manage Frequently Asked Questions (FAQ)</span>
                    </p>
                </div>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button
                        onClick={() => router.push("/superadmin/content/got-questions/add-edit")}
            ...
            >
                {filteredItems.length === 0 ? (
                    <div className="text-center py-16">
                        ...
                        <p className="text-gray-500 mb-6">Create your first FAQ item.</p>
                        {!searchTerm && (
                            <button
                                onClick={() => router.push("/superadmin/content/got-questions/add-edit")}
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="bg-gray-50 border-b border-gray-200">
                                    <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-1/3">Question</th>
                                    <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-1/2">Answer</th>
                                        ...
                                        >
                                            <td className="px-6 py-4 align-top">
                                                <div className="flex items-start gap-2">
                                                    <HelpCircle size={16} className="text-blue-500 mt-1 flex-shrink-0" />
                                                    <span className="font-semibold text-gray-900">{item.question}</span>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 align-top">
                                                <div className="flex items-start gap-2">
                                                    <MessageCircle size={16} className="text-green-500 mt-1 flex-shrink-0" />
                                                    <p className="text-sm text-gray-600 line-clamp-3" title={item.answer}>
                                                        {item.answer}
                                                    </p>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium align-top">
                                                <div className="flex items-center justify-end gap-2">
                                                    <button
                                                        onClick={() => router.push(`/superadmin/content/got-questions/add-edit?id=${item._id || item.id}`)}
                                                    ...
                                                    <button
                                                        onClick={() => handleDelete(item._id || item.id!, item.question)}
...
export default GotQuestionsPage;
