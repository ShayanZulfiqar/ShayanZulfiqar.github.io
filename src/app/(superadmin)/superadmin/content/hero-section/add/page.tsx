...
import { ArrowLeft, Save, Sparkles, Palette, Zap } from "lucide-react";
import { HexColorPicker } from "react-colorful";
import { createHeroSectionAction } from "@/store/slices/landingSlice";
import { useAppDispatch, useAppSelector } from "@/store/hooks";
import AlertDialog from "@/utils/AlertDialog";

const AddHeroSectionPage = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const resultAction = await dispatch(createHeroSectionAction(formData));

            const endTime = performance.now();
            console.log(`Create API Response Time: ${(endTime - startTime).toFixed(2)}ms`);

            if (createHeroSectionAction.fulfilled.match(resultAction)) {
                AlertDialog(
                    "Success!",
                ...
                throw new Error(resultAction.payload as string || "Failed to create hero section");
            }
        } catch (error: any) {
            AlertDialog(
                "Error!",
                error.message || "Failed to create hero section",
                "error",
                3000,
                false,
                false
            ...
            >
                ...
                <div>
                    ...
                    <p className="text-gray-600 flex items-center gap-2 mt-1">
                        ...
                        <span>Create a new hero section for your landing page</span>
                    </p>
                </div>
            </motion.div>
                                    ...
                                    >
                                        <div className="absolute z-10 bg-white p-4 rounded-xl shadow-2xl border border-gray-200">
                                            ...
                                            <button
                                                ...
                                                className="mt-3 w-full px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition-all font-medium text-sm"
                                            >
                                                Done
                                            </button>
                                        </div>
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onChange={handleChange}
                                placeholder="linear-gradient(...)"
                                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-transparent font-mono text-sm"
                            />
                            <div
                                className="mt-2 h-12 rounded-lg border-2 border-gray-300"
...
export default AddHeroSectionPage;
