...
import { fetchCoreValues, deleteCoreValueAction } from "@/store/slices/landingSlice";
import AlertDialog from "@/utils/AlertDialog";

const CoreValuesPage = () => {
  const router = useRouter();
  const dispatch = useAppDispatch();
  const { data: coreValues, loading, error } = useAppSelector((state) => state.landing.coreValues);
  ...
  const handleDelete = (id: string, title: string) => {
    AlertDialog(
      ...
      async () => {
        try {
          const resultAction = await dispatch(deleteCoreValueAction(id));
          if (deleteCoreValueAction.fulfilled.match(resultAction)) {
            AlertDialog(
              "Success!",
            ...
            throw new Error(resultAction.payload as string || "Failed to delete core value");
          }
        } catch (error: any) {
          AlertDialog(
            "Error!",
            error.message || "Failed to delete core value",
            "error",
            3000,
            false,
            false
  ...
  // Filter core values based on search
  const filteredCoreValues = coreValues.filter((value) =>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 text-lg">Loading core values...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6 pb-8">
      ...
      >
        <div>
          ...
          <p className="text-gray-600 flex items-center gap-2">
            ...
            <span>Manage your organization's core values</span>
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => router.push("/superadmin/content/core-values/add")}
      ...
      >
        <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl p-6 border border-purple-200">
          <p className="text-purple-600 text-sm font-semibold mb-1">Total Values</p>
          <p className="text-3xl font-bold text-purple-700">{coreValues.length}</p>
        </div>
        <div className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-xl p-6 border border-blue-200">
          <p className="text-blue-600 text-sm font-semibold mb-1">Filtered Results</p>
          <p className="text-3xl font-bold text-blue-700">{filteredCoreValues.length}</p>
        </div>
        <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-6 border border-green-200">
          <p className="text-green-600 text-sm font-semibold mb-1">Last Updated</p>
          <p className="text-lg font-bold text-green-700">
            {coreValues.length > 0 ? new Date(coreValues[0].updatedAt || "").toLocaleDateString() : "N/A"}
          </p>
        </div>
      </motion.div>
      ...
      >
        {filteredCoreValues.length === 0 ? (
          <div className="text-center py-16">
            ...
            <h3 className="text-xl font-semibold text-gray-700 mb-2">No Core Values Found</h3>
            <p className="text-gray-500 mb-6">
              {searchTerm ? "Try adjusting your search terms" : "Get started by adding your first core value"}
            </p>
            {!searchTerm && (
              <button
                onClick={() => router.push("/superadmin/content/core-values/add")}
                  ...
                  {filteredCoreValues.map((value, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <p className="text-sm font-bold text-gray-800">{value.title}</p>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-center gap-3">
                          ...
                          <div
                            ...
                            title={`Background: ${value.bgColor}`}
                          />
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-center gap-2">
                          <button
                            onClick={() => router.push(`/superadmin/content/core-values/edit/${value._id}`)}
                          ...
                          <button
                            onClick={() => handleDelete(value._id!, value.title)}
